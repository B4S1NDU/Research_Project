...
class AIExplainer:
    ...
    def _compare_with_trained_model(self, artifact1: Dict, artifact2: Dict) -> Dict:
        ...
        Similarity scores / similarities / differences come from the sentence-transformer
        subprocess. The comparison narrative is generated by the T5 model already loaded
        in this process (self._artifact_ai_explainer).
        """
        try:
            if not (self.model_service and self.model_service.is_ready):
                raise Exception("Model service not available")

            # 1. Get similarity data from sentence-transformer subprocess
            comparison = self.model_service.compare(artifact1['id'], artifact2['id'])

            if not comparison or 'error' in comparison:
                raise Exception(f"Model service error: {comparison.get('error') if comparison else 'no response'}")

            # 2. Generate comparison narrative using T5 in this process.
            #    Wait briefly in case T5 is still finishing its warmup.
            if not self._model_ready:
                print("⏳ Waiting for T5 model to finish loading...")
                for _ in range(30):
            ...
            comparison_text = comparison.get('comparison', '')  # keep template as fallback
            ...
            if self._model_ready and self._artifact_ai_explainer:
                try:
                    ...
                    if t5_text and len(t5_text) > 50:
                        ...
                        # similarity score computed by the sentence-transformer model
                        score = float(comparison.get('similarity_score', 0))
                        if score >= 60:
                            insight = (
                                f"With a semantic similarity of {score:.1f}%, these artifacts demonstrate "
                                f"how cultural practices and artistic expressions can transcend geographical "
                                f"boundaries. Both pieces serve as important witnesses to human creativity "
                                f"and cultural exchange."
                            )
                        else:
                            insight = (
                                f"Despite their differences (similarity: {score:.1f}%), comparing these "
                                f"artifacts reveals universal human needs for ceremonial objects, artistic "
                                f"expression, and cultural identity. Each represents the unique genius of "
                                f"its originating culture."
                            )
                    ...
                    print(f"⚠ T5 error: {t5_err} — keeping template text")
            else:
